...
## Предупреждение `setlocale: LC_ALL: cannot change locale (en_US.UTF-8)`

Значит в SSH/окружении задана локаль `en_US.UTF-8`, а на сервере она не сгенерирована. Скрипт стартует с `C.UTF-8` через shebang; если предупреждение осталось, исправьте систему:
...
Или однократно без правки системы: `LC_ALL=C.UTF-8 LANG=C.UTF-8 ./start-mtproxy.sh --show-link`.
